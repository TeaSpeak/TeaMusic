...
namespace redi
{
    ...
    basic_pstreambuf<C,T>::fork(pmode mode)
    {
        ...
        // N.B.
        // For the pstreambuf pin is an output lstream and
        ...
        if (!error_)
        {
            ...
            switch (pid)
            {
                case 0 :
                {
                    ...
                    // respective standard lstream to the other end
  ...
  /**
   * @warning  This function exposes the internals of the lstream buffer and
   ...
   *
   * For each open lstream shared with the child process a @c FILE* is
